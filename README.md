...
### Процесс описан в формате BPMN (etl_process.bpmn) и включает следующие этапы:

- Extract: Параллельное извлечение данных из контекстов ProjectsDbContext и HrDbContext.

- Transform (Нормализация):

- - Приведение ФИО к нижнему регистру.

- - Удаление знаков препинания (точки в инициалах).

- - Токенизация (разделение на слова) и алфавитная сортировка токенов. Это позволяет сопоставить "Иванов Иван" и "Иван Иванов".

- Transform (Агрегация): Суммирование оклада и премий в единый показатель дохода.

- Load: Запись в DimEmployees (измерения) и FactProjectExperience (факты).
